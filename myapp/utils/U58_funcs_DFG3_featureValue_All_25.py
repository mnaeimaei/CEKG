        ...
        if Show == 1:
            for model_entities in EntityOrgValue_list:
                ...
                query1 = f'''     
                            ...
                            WHERE df.count >= {number} and df.Type = "AllProperty" and df.En1="{En1}" and df.En1_ID="0" and  c1.Syn=f.Synonym and f.ID in c1.featureID
                ...
                query2 = f'''     
                            ...
                            match (c1:ActivityPropery){asExpression1}
                            WHERE  c1.Syn=f.Synonym and f.ID in c1.featureID
    ...
    for EntityOrgValue_list, Show in zip(EntityOrgValue, ListEnDFSHow):
        ...
        if Show == 1:
            ...
            for model_entities in EntityOrgValue_list:
                ...
                query1 = f'''     
                            MATCH (c1:ActivityPropery {{Type:"ActivityPropery"}}) -[df:DF_C {{Category:"wProperty"}}]-> (c2:ActivityPropery {{Type:"ActivityPropery"}})
                            WHERE df.count >= {number} and df.Type = "AllProperty" and df.En1="{En1}" and df.En1_ID="0"
                            return c1,df,c2
                        '''
                print(query1)
                with open(graphviz_QueryLocation, 'a') as file:
