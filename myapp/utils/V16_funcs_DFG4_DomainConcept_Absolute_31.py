...
def DFC_based_on_Origins(final_DFG_List, ID_Colors, ListEnDFSHow, count, case_selector, case_selector_activation,
                         ...
                         Type4_Domain_selection, Type4_Domain_ID, graphviz_QueryLocation):
        ...
        if Show == 1:
            for model_entities in EntityOrgValue_list:
                ...
                query1 = f'''     
                        ...
                        WHERE df.count >= {number} and df.Type = "Absolute" and df.En1="{En1}" and df.En1_ID="{En1_ID}" {actExpression1} {domExpression1}
            ...
            for model_entities in EntityOrgValue_list:
                ...
                if case_selector_activation == True:
                    query1 = f'''     
                            ...
                            MATCH (d1)<-[:TYPE_OF]-(c1:Activity {{Type:"Activity"}})
                            MATCH (c2:Activity {{Type:"Activity"}})-[:TYPE_OF]->(d2)
                            WHERE df.count >= {number} and df.Type = "Absolute" and df.En1="{En1}" and df.En1_ID="{En1_ID}" and {case_selector} {domExpression1}
                            return c1,df,c2,d1,d2
                            '''
                    ...
                    query1 = f'''     
                            ...
                            MATCH (d1)<-[:TYPE_OF]-(c1:Activity {{Type:"Activity"}})
                            MATCH (c2:Activity {{Type:"Activity"}})-[:TYPE_OF]->(d2)
                            WHERE df.count >= {number} and df.Type = "Absolute" and df.En1="{En1}" and df.En1_ID="{En1_ID}" {domExpression1}
                            return c1,df,c2,d1,d2
                            '''
                # print(query1)
                with open(graphviz_QueryLocation, 'a') as file:
