...
def DFC_based_on_Origins(final_DFG_List, ID_Colors, ListEnDFSHow, count, case_selector, case_selector_activation,
                         c_white, c_black, dot, driver, DomainColors, domainColTitle, activityScenario, colTitle,
                         case_selector2, Type4_Domain_selection, Type4_Domain_ID, graphviz_QueryLocation):
            ...
            for model_entities in EntityOrgValue_list:
                ...
                if Type4_Domain_selection == True:
                    domExpression1 = " and d1." + domainColTitle + " in " + str(
                        Type4_Domain_ID) + " and d2." + domainColTitle + " in " + str(Type4_Domain_ID)
                ...
                query1 = f'''     
                        ...
                        WHERE df.count >= {number} and df.Type = "AbsoluteProperty" and df.En1="{En1}" and df.En1_ID="{En1_ID}" and  c1.Syn=f.Synonym and f.ID in c1.featureID {actExpression1}   {domExpression1}
                ...
                query2 = f'''     
                        ...
                        MATCH (d1)<-[:TYPE_OF]-(c1:ActivityPropery {{Type:"ActivityPropery"}})
                        match (c1:ActivityPropery){asExpression1}
                        WHERE c1.Syn=f.Synonym and f.ID in c1.featureID {actExpression2}   {domExpression2}
                ...
                if record2:
                    ...
                    for i in range(len(record2)):
                        ...
                        dot.node(str(f1_Neo4J_ID_new_2nd[i]), c1_label, color=Node_Around_Color, penwidth="2",
                                 style="filled",
...
def DFC_Adding_Entities(final_DFG_List, ID_Colors, ListEnDFSHow, count, case_selector, case_selector_activation,
                        case_selector_list, c_white, c_black, dot, driver, Type4_Domain_selection, Type4_Domain_ID,
                        domainColTitle, graphviz_QueryLocation):
    list3 = []
    for EntityOrgValue_list, Show in zip(final_DFG_List, ListEnDFSHow):
        if Show == 1:
            list1 = []
...
def Adding_Domains_Colors_Hint(final_DFG_List_Abs, ListEnDFSHow, count, case_selector, case_selector_activation,
                               DomainColors, c_white, c_black, dot, driver, domainColTitle, Type4_Domain_selection,
                               Type4_Domain_ID, graphviz_QueryLocation):
    list3 = []
