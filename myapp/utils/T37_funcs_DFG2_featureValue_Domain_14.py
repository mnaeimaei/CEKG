...
def DF_based_on_Entities(entityList, entityIDlists, EntOrgAbrNum, EntitiesColors, ListEnDFSHow, case_selector1,
                         ...
                         domainColTitle,activityScenario, colTitle,Type1_Domain_selection , Type1_Domain_ID, graphviz_QueryLocation):
    for eachEntity, EntityIDList, Color, Show in zip(entityList, entityIDlists, EntitiesColors, ListEnDFSHow):
        if Show == 1:
            for eachID in EntityIDList:
                ...
                if Type1_Domain_selection == False:
                    domExpression1 = " "
                    domExpression2 = " "
                    domExpression3 = " "

                if Type1_Domain_selection == True:
                    domExpression1 = " and d1." + domainColTitle + " in " + str(Type1_Domain_ID) + " and d2." + domainColTitle + " in " + str(Type1_Domain_ID)
                    domExpression2 = " and d1." + domainColTitle + " in " +  str(Type1_Domain_ID)
                    domExpression3 = " and d." + domainColTitle + " in " +  str(Type1_Domain_ID)
                ...
                query1 = f'''     
                    ...
                    match (e1)-[:OBSERVED]->(a1)-[:TYPE_OF]->(d1)
                    match (e2)-[:OBSERVED]->(a2)-[:TYPE_OF]->(d2)
                    match (e1)-[:OBSERVED]->(a1){asExpression1}
                    match (e2)-[:OBSERVED]->(a2){asExpression2}
                    WHERE n1.ID="{eachID}" AND n2.ID="{eachID}" and r.ID="{eachID}"  {actExpression2}  {domExpression1} 
                    return distinct f, a1,e1, df, e2,a2,d1,d2 {asExpression3}
                ...
                if record1:
                    ...
                    e1_domain = [item[6][domainColTitle] for item in record1]
                    e2_domain = [item[7][domainColTitle] for item in record1]

                    if activityScenario=="Activity_Label":
                        e1_Activity = [item[1][colTitle] for item in record1]
                        e2_Activity = [item[5][colTitle] for item in record1]
                    else:
                        e1_Activity = [item[8][colTitle] for item in record1]
                        e2_Activity = [item[9][colTitle] for item in record1]
                    ...
                    for i in range(len(record1)):
                        ...
                        print("e1_Neo4J_ID=", str(e1_Neo4J_ID[i]))
                        print("e2_Neo4J_ID=", str(e2_Neo4J_ID[i]))
                        print("Rel_label=", Rel_label)
                        Node_Color1 = Node_Color
                        Node_Color2 = Node_Color
                        print(DomainColors)
                        for Value in DomainColors:
                            if e1_domain[i] == Value[0]:
                                Node_Color1 = Value[1]
                            if e2_domain[i] == Value[0]:
                                Node_Color2 = Value[1]

                        dot.node(str(f1_Neo4J_ID_new_2nd[i]), f_node[i], color=Node_Around_Color, penwidth="2",
                                 style="filled",
                                 fillcolor=Node_Color2)
                        dot.node(str(e2_Neo4J_ID[i]), e2_label, color=Node_Around_Color, penwidth="2",
                                 style="filled",
                                 fillcolor=Node_Color2)
...
##################################
                ...
                query2 = f'''     
                        ...
                        match (e1)-[:OBSERVED]->(a1)-[:TYPE_OF]->(d1)
                        match (e1)-[:OBSERVED]->(a1){asExpression1}
                        WHERE n1.ID="{eachID}"   {actExpression1}  {domExpression2} 
                        return distinct f,e1,a1,d1{asExpression4}  
                                     '''
                ...
                if record2:
                    ...
                    e1_Neo4J_ID = [item[1].id for item in record2]
                    e1_Event = [item[1]["Event"] for item in record2]
                    e1_domain = [item[3][domainColTitle] for item in record2]

                    if activityScenario=="Activity_Label":
                        e1_Activity = [item[2][colTitle] for item in record2]
                    else:
                        e1_Activity = [item[4][colTitle] for item in record2]
                    ...
                    for i in range(len(record2)):
                        ...
                        dot.edge(str(e1_Neo4J_ID[i]), str(f1_Neo4J_ID_new_2nd[i]), label=f_edge[i],
                                 color=Edge_Color,
                                 penwidth=pen_width, fontname="Helvetica", fontsize="8",
                                 fontcolor=edge_font_color)
...
def DF_based_on_ID(entityList, refEntityIDlists, ListEnOrgRelDFShow, case_selector1, case_selector2,
                   case_selector_activation, c_white, c_black, SubEntities_Color, dot, driver,Type1_Domain_selection , Type1_Domain_ID,domainColTitle, graphviz_QueryLocation):
    print(refEntityIDlists)
    for eachEntity, EntityIDList, Show in zip(entityList, refEntityIDlists, ListEnOrgRelDFShow):
        ...
        if Show == 1 and EntityIDList:
            for eachID in EntityIDList:
                ...
                if Type1_Domain_selection == True:
                    domExpression1 = " and d1." + domainColTitle + " in " + str(Type1_Domain_ID) + " and d2." + domainColTitle + " in " + str(Type1_Domain_ID)
                    domExpression4 = "where d1." + domainColTitle + " in " + str(Type1_Domain_ID) + " and d2." + domainColTitle + " in " + str(Type1_Domain_ID)
                ...
                if case_selector_activation == True:
                    query = f'''
                                        ...
                                        match (e1)-[:OBSERVED]->(a1)-[:TYPE_OF]->(d1) 
                                        match (e2)-[:OBSERVED]->(a2)-[:TYPE_OF]->(d2)
                                        WHERE {case_selector2} {domExpression1} 
...
def Adding_Entities_ID_ForFirstEvent(entityList, entityIDlists, EntitiesColors, ListEnDFSHow, case_selector1,
                                     case_selector_activation, c_white, c_black, dot, driver,Type1_Domain_selection , Type1_Domain_ID,domainColTitle, graphviz_QueryLocation):
    for eachEntity, EntityIDList, Color, Show in zip(entityList, entityIDlists, EntitiesColors, ListEnDFSHow):
        if Show == 1:
            for eachID in EntityIDList:
                ...
                query2 = f'''
                        ...
                        match (e)-[:OBSERVED]->(a)-[:TYPE_OF]->(d) 
                        WHERE n1.Type = "{eachEntity}" and n1.ID="{eachID}"  
                        {actExpression1}
                        {domExpression3} 
                        return e, n1
...
def Adding_Entities(entityList, entityIDlists, EntitiesColors, ListEnDFSHow, c_white, c_black, dot, driver,
                    case_selector2, case_selector_activation,Type1_Domain_selection , Type1_Domain_ID,domainColTitle, graphviz_QueryLocation):
    list3 = []
        ...
        if Show == 1:
            ...
            for eachID in EntityIDList:
                ...
                query1 = f'''     
                    ...
                    match (e1)-[:OBSERVED]->(a1)-[:TYPE_OF]->(d1)
                    match (e2)-[:OBSERVED]->(a2)-[:TYPE_OF]->(d2)
                    WHERE n1.ID="{eachID}" AND n2.ID="{eachID}"  {actExpression2}  {domExpression1} 
...
def Adding_Domains_Colors_Hint(entityList, entityIDlists, ListEnDFSHow, case_selector1, case_selector_activation,
                               DomainColors, c_white, c_black, dot, driver, domainColTitle,Type1_Domain_selection , Type1_Domain_ID, graphviz_QueryLocation):
    list3 = []
    for eachEntity, EntityIDList, Show in zip(entityList, entityIDlists, ListEnDFSHow):
        if Show == 1:
            list1 = []
            for eachID in EntityIDList:
                ...
                query1 = f'''     
                    match(e1: Event) -[df: DF{{Type:\"{eachEntity}\"}}]-> (e2:Event) -[: CORR]-> (n1:{eachEntity} {{Type:\"{eachEntity}\"}})
                    match (e1)-[:OBSERVED]->(a1)-[:TYPE_OF]->(d1)
                    match (e2)-[:OBSERVED]->(a2)-[:TYPE_OF]->(d2)
                    WHERE n1.ID="{eachID}"   {actExpression1}  {domExpression1} 
                    return d1,e1, df, e2,d2
                ...
                if record1:
                    e1_domain = [item[0][domainColTitle] for item in record1]
                    list2.extend(e1_domain)
                    e2_domain = [item[4][domainColTitle] for item in record1]
                    list2.extend(e2_domain)
