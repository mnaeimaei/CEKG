...
if myInput == 'main_Entities_plus_Disorder': #2
    ...
    if selenium==True:
        print("\n Neo4J Query to SVG:")
        runingQuery = f'''
        MATCH ( e: Event ) -[:CORR{{Scenario:"2"}}]-> ( p: Disorder  ) return p limit 25;
        '''
...
if myInput == 'main_Entities_plus_ICD': #3
    ...
    if selenium==True:
        print("\n Neo4J Query to SVG:")
        runingQuery = f'''
        MATCH ( e: Event ) -[:CORR{{Scenario:"2"}}]-> ( p: Clinical  ) return p limit 25;
        '''
        svgFunction.exportNeo4jSVG(runingQuery, driverUserName, driverPassword)
        svgFunction.moveSVG(outDir, fileName)
        savingTxt4 = svgLog + "/" + fileName + "_svg_location.txt"
...
if myInput == 'main_Entities_plus_SCT': #5
    ...
    if selenium==True:
        print("\n Neo4J Query to SVG:")
        runingQuery = f'''
        MATCH ( e: Event ) -[:CORR{{Scenario:"2"}}]-> ( p: Concept  ) return p limit 25;
        '''
        svgFunction.exportNeo4jSVG(runingQuery, driverUserName, driverPassword)
        svgFunction.moveSVG(outDir, fileName)
        savingTxt4 = svgLog + "/" + fileName + "_svg_location.txt"
