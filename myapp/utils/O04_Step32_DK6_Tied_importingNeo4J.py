...
if DFG in list1:
    ...
    if selenium==True:
        print("\n Neo4J Query to SVG:")
        runingQuery = f'''
        Match p=(a)-[:TYPE_OF{{Scenario:"1"}}]->(d:Domain) return p limit 25;
        '''
...
if DFG in list2:
    ...
    if selenium==True:
        print("\n Neo4J Query to SVG:")
        runingQuery = f'''
        Match p1=(a)-[:TYPE_OF{{Scenario:"2"}}]->(c:Concept)           Match p2= ( f ) -[:TIED]-> ( s )         return p1,p2 limit 25;
        '''
        svgFunction.exportNeo4jSVG(runingQuery, driverUserName, driverPassword)
        svgFunction.moveSVG(outDir, fileName)
        savingTxt4 = svgLog + "/" + fileName + "_svg_location.txt"
...
if DFG in list3:
    ...
    if selenium==True:
        print("\n Neo4J Query to SVG:")
        runingQuery = f'''
        Match p1=(a)-[:TYPE_OF{{Scenario:"3"}}]->(c)           Match p2= ( f ) -[:TIED]-> ( s )         return p1,p2 limit 25;
