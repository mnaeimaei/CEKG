...
def DFC_based_on_Origins (EntityOrgValue, ID_Colors, count, c_white, c_black, dot, driver,DomainColors,domainColTitle,activityScenario, colTitle,case_selector_activation1,case_selector_list1,case_selector1,case_selector_activation2,case_selector_list2,case_selector2, case_selector3, case_selector4,Type5_Domain_selection, Type5_Domain_ID, graphviz_QueryLocation):
    for model_entities in EntityOrgValue:
        ...
        query1 = f'''     
                ...
                MATCH (c1:ActivityPropery)-[:TYPE_OF]->(d1) 
                MATCH (c2:ActivityPropery)-[:TYPE_OF]->(d2)
                {asExpression1}
                {asExpression2}
                WHERE df.count >= {number} and df.Type = "RelativeProperty" and df.En1="{En1}" and df.En2="{En2}" and df.En1_ID="{En1_ID}" and  c1.Syn=f.Synonym and f.ID in c1.featureID {actExpression1} {actExpression2}  {domExpression1}
...
def DFC_Adding_Entities (final_DFG_List, count, ID_Colors, c_white, c_black, dot, driver,case_selector_activation1,case_selector_list1,case_selector1,case_selector_activation2,case_selector_list2,case_selector2, domainColTitle,Type5_Domain_selection, Type5_Domain_ID, graphviz_QueryLocation):
    ...
    for model_entities in final_DFG_List:
        ...
        query1 = f'''     
                    ...
                    MATCH (d1)<-[:TYPE_OF]-(c1:ActivityPropery {{Type:"ActivityPropery"}})
                    MATCH (c2:ActivityPropery {{Type:"ActivityPropery"}})-[:TYPE_OF]->(d2)
                    WHERE df.count >= {number} and df.Type = "RelativeProperty" and df.En1="{En1}" and df.En2="{En2}" and df.En1_ID="{En1_ID}" {actExpression1} {actExpression2} {domExpression1}
    ...
    for Entity_Alias, posit in zip(final_DFG_List, range(1, 700)):
        ...
        #print(En1)
        En2 = Entity_Alias[1]
        En1_ID = Entity_Alias[2]
        Color_ID = Entity_Alias[3]
        color = ID_Colors[Color_ID]
