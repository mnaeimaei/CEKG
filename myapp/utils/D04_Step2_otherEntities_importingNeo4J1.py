...
svgLog = tempLoc + "/" + '02_OtherEntities'
...
if step_createProperty:
    ...
    if selenium==True:
        print("\n Neo4J Query to SVG:")
        runingQuery = f'''
        WITH {NodeEntity} AS labels         UNWIND labels AS label          CALL apoc.cypher.doIt('MATCH (p:`' + label + '`) RETURN p AS node', {{}}) YIELD value         RETURN value.node AS node;
        '''
        print(runingQuery)
